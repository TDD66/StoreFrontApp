...
const StoreContainer = () => {
    ...
    const fetchCustomers = async () => {
        const response = await fetch('http://storefrontapp-production.up.railway.app:8080/customers');
        const data = await response.json();
        setCustomers(data);
    }

    const fetchItems = async () => {
        const response = await fetch('http://storefrontapp-production.up.railway.app:8080/items');
        const data = await response.json();
        setItems(data);
    }

    const fetchOrders = async () => {
        const response = await fetch('http://storefrontapp-production.up.railway.app:8080/orders');
        const data = await response.json();
        setOrders(data);
    }

    const fetchReviews = async () => {
        const response = await fetch('http://storefrontapp-production.up.railway.app:8080/reviews');
        const data = await response.json();
        setReviews(data);
    }

    const fetchOrderedItems = async () => {
        const response = await fetch('http://storefrontapp-production.up.railway.app:8080/ordered-items');
        const data = await response.json();
    ...
    const patchReview = async (amendedReview, reviewId) => {
        const response = await fetch(`http://storefrontapp-production.up.railway.app:8080/reviews/${reviewId}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
    ...
    const completeOrder = async (basketItems, customerId, address) => {
        ...
        const newOrderResponse = await fetch("http://storefrontapp-production.up.railway.app:8080/orders", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(newOrder)
        });
